...
contract Insurance is Ownable {
  ...
  function updateProfiles(bytes32 _protocol, uint256 _maxFundsCovered, uint256 _percentagePremiumPerBlock, uint256 _premiumLastPaid) external onlyOwner {
    ...
    profiles[_protocol] = ProtocolProfile(_maxFundsCovered, _percentagePremiumPerBlock);
  ...
  function premiumPerBlock(bytes32 _protocol) public view returns(uint256) {
    ...
    return coveredFunds(_protocol).mul(p.percentagePremiumPerBlock).div(10**18);
  }
